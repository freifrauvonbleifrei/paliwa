...
template <class ChunkSpanType, class ExecSpace = Kokkos::DefaultExecutionSpace>
class AdaptedData {
private:
  ...
  using chunk_type = ddc::Chunk<value_type, domain_type, allocator_type>;
...
public:
  ...
  [[nodiscard]] span_type view() const noexcept { return m_span; }
...
private:
  ...
  template <class ES, class CST>
  friend void adapter_out(CST &dst, AdaptedData<CST, ES> const &adapted,
                          ES instance);
};

template <class ExecSpace = Kokkos::DefaultExecutionSpace, class ChunkSpanType>
[[nodiscard]]
auto adapter_in(ChunkSpanType const &src, ExecSpace instance = ExecSpace()) {
  ...
  if constexpr (accessible) {
    ...
    adapted.m_chunk.emplace(
        "adapter_buffer", src.domain(),
        ddc::KokkosAllocator<value_type, typename ExecSpace::memory_space>());

    adapted.m_span = ddc::parallel_deepcopy(instance, *adapted.m_chunk, src);
  }
...
template <class ExecSpace = Kokkos::DefaultExecutionSpace, class ChunkSpanType>
void adapter_out(ChunkSpanType &dst,
  ...
  ddc::parallel_deepcopy(instance, dst, *adapted.m_chunk);
}
